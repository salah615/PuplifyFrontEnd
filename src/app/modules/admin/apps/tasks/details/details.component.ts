...
export class TasksDetailsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  deleteTask(): void {
    ...
    confirmation.afterClosed().subscribe((result) => {
      ...
      if (result === "confirmed") {
        ...
        const index = this._tasksListComponent.tasks.findIndex(
          (item) => item.id === id
        );

        this._tasksListComponent.tasks.splice(index, 1);
  ...
  private refreshRoute(): void {
    // Get the current URL and navigate to it to force a reload
